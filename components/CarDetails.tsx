    ...
    if (isOpen) {
      const fetchImages = async () => {
        try {
          ...
          const mainImage = await generateCarImageUrl(car);
                ...
                <Dialog.Panel className='relative w-full max-w-lg max-h-[90vh] overflow-y-auto transform rounded-2xl bg-white p-6 text-left shadow-xl transition-all flex flex-col gap-5'>
                  ...
                  <div className='flex-1 flex flex-col gap-3'>
                    <div className='relative w-full h-40 bg-pattern bg-cover bg-center rounded-lg'>
                      {mainImageUrl ? (
                        <Image src={mainImageUrl} alt='car model' fill priority className='object-contain' />
                      ) : (
                        <p>Loading image...</p>
                      )}
                    </div>
                    ...
                    <div className='flex gap-3'>
                      {additionalImageUrls.map((url, index) => (
                        <div key={index} className='flex-1 relative w-full h-24 bg-primary-blue-100 rounded-lg'>
                          <Image src={url} alt={`car model ${index + 1}`} fill priority className='object-contain' />
                        </div>
                      ))}
                    </div>
                  </div>
